...
uses
  ...
  Dialogs, RzButton, ComCtrls, RzListVw, mysql, ExtCtrls, ImgList, DateUtils,
  mmSystem;

type
  TFormKitchenInfo = class(TForm)
...
var
  FormKitchenInfo: TFormKitchenInfo;

implementation

uses myfunction, dbfunction, mfmKitchenManager, fmHasten;

{$R *.dfm}

procedure TFormKitchenInfo.checkHastenInfo;
var
  sql: string;
...
procedure TFormKitchenInfo.FormCreate(Sender: TObject);
begin
  connectMySql(LibHandle);
  self.loadBarItemsInfo;
end;

procedure TFormKitchenInfo.FormDestroy(Sender: TObject);
var
  f:TFormKitchenManager;
begin
  ...
  f:= TFormKitchenManager(self.Owner);
  if f<>nil then
    f.fmKitchen:=nil;
end;

procedure TFormKitchenInfo.CreateParams(var Params: TCreateParams);
begin
...
procedure TFormKitchenInfo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  self.Timer1.Enabled:=false;
  self.Destroy;
end;

procedure TFormKitchenInfo.loadBarItemsInfo;
var
  sql: string;
  MYSQL_ROW: PMYSQL_ROW;
  item:TListItem;
...
begin
  ...
  sql := 'select a.id,a.menuidx,a.tablename,b.itemname,a.nums,b.itemunit,a.usertips,a.creationtime from consumption a left join menus b on b.menuidx=a.menuidx where b.makefrom=1 and a.payidx=0 and a.statusflag=0 order by a.creationtime';
  queryMySql(sql,LibHandle, mySQL_Res);
  if mysql_num_rows(mySQL_Res)>0 then begin
    repeat
      MYSQL_ROW := mysql_fetch_row(mySQL_Res);
...
procedure TFormKitchenInfo.RzListViewAdvancedCustomDrawItem(Sender:
...
begin
  ...
  for I := 0 to ls.Count - 1 do begin
    if ls[i]=td.tableName then begin
      ...
      Sender.Canvas.Font.Size:=28;
      wav:=ExtractFilePath(Application.ExeName) + 'hasten.wav';
      if FileExists(wav) then begin
        playsound(PWideChar(wav),0,SND_ASYNC);
      end;
      break;
    end;
  end;
...
procedure TFormKitchenInfo.Timer1Timer(Sender: TObject);
begin
  checkHastenInfo;
  loadBarItemsInfo;
end;
